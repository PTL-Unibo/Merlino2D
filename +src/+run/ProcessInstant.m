...
RHO_CELLS = src.const.e*sum(reshape(N_CELLS,Nc,ns).*qs,2);
...
RHO_NODES = src.const.e*sum(reshape(N_NODES,Nn,ns).*qs,2);
...
J_faces = src.const.e * reshape(GAMMA_DOT_N, Nf, ns) .* qs;
for i = 1:msh.dim_bID
