...
n_c = y(1:p.ns*p.Nc);
phi = y(p.ns*p.Nc+p.Nd+1:end-1);
sc = y(end);

% Compute electric field in Td
aux_BC_el = p.M_get_aux_BC_el * p.BCEL_VAL * sc;
Ecx = p.Eint2Ec * (p.phi2Ex * phi + p.aux2Ex * aux_BC_el);
Ecy = p.Eint2Ec * (p.phi2Ey * phi + p.aux2Ey * aux_BC_el);
