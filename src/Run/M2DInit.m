...
if flag == "run"
    ...
    y0 = [N0(:); sigma0; zeros(2*msh.Nf,1); phi0; vc0]; % wrong initial condition

    % Create Jacobian sparsity pattern ----------------------------------------
    JPattern = CreateJpattern(msh, qs, Kelet, Flux2N, phi2En, rho2RHS, phi2Ex, phi2Ey, aux2RHS, M_get_aux_BC_el, p.BCEL_VAL, g2Is'.*Ex_1, g2Is'.*Ey_1);
    [i,j,s] = find(JPattern);
...
if flag == "run"
    ...
    y0 = [N0(:); sigma0; J0; phi0; vc0]; % consistent initial condition
    y0 = y0(ppp);
