...
\begin{document}
	...
	that is inside the \textbf{src/} folder.
	By running this script, you will be firstly asked to select the \textbf{gmsh.exe} executable that you should have previously installed on your computer.
	After, you will be asked to select the \textbf{Code/} folder of the \href{https://github.com/LoKI-Suite/LoKI-B}{LoKI-B} repository, that you should have cloned on your device if you wish to have the option of using the Boltzmann solver for computing swarm parameters and rate coefficients.
	
	If the folder containing the project is moved to another location, you need to run \textbf{Merlino2D\_startup.m} again.
	
	\subsection{Verification}
	If the installation was successful, you can now run all the examples contained in the \textbf{cases/} folder (if you also moved the \textbf{Air.in} file inside the \textbf{Code/Input/} folder of the LoKI-B repository). 
	We suggest that you start by running the script \textbf{c\_Diffusion.m}, which should not take a long time and does not require to move the \textbf{Air.in} file.
	...
	\matlabs{Photoionization is OFF}
	
	\matlabs{Initialization finished}
	
	\matlabs{Updated photoionization, maximum value = 0.000000e+00}
...
\noindent This will add to the MATLAB path the folder \textbf{src/} that contains the code. 
...
\item \matlabs{"sph"}, to plot the photoionization source term
...
\matlabs{out\_pp = Load("my\_result.mat");}
	...
	Note also that \textbf{Merlino2D identifies as dielectric interfaces only lines assigned a numerical identifier greater than or equal to 1000}. 
%	\footnote{This apparently random number actually corresponds to the first four digits of the vacuum permittivity $\varepsilon_0 = \SI{8.8541878188e-12}{\farad\per\meter}$.}.
	For this reason, in the \textbf{.geo} file, the yellow line can be labeled, for example, with the identifier 1005 using the \matlabs{Physical Curve} command.
	...
	\noindent With this combination of parameters you are imposing a constant number density of \SI{1e5}{\per\cubic\meter} for the species \matlabs{"e"} at the third boundary, and a time-dependent expression for computing the \textbf{outgoing} flux of \matlabs{"e"} at the second boundary and of \matlabs{"N2+"} at the third boundary. Notice that \matlabs{NaN} have been placed in \seclink{bcval}{BC\_VAL} at positions corresponding to \matlabs{"GorinLike"} and \matlabs{"FreeDriftFlow"} flags. This was done to highlight that, for those species and boundaries where a \matlabs{"GorinLike"} or a \matlabs{"FreeDriftFlux"} condition has been used, the value inserted in the corresponding position of \seclink{bcval}{BC\_VAL} will be completely ignored.
	...
	\[
	n_i(x,y) = \boldsymbol{\mathrm{A}}[i] \, \mathrm{exp}\!\!\left(-\left(\frac{(x-\boldsymbol{\mathrm{x0}}[i])^2}{(\boldsymbol{\mathrm{sigma\_x}}[i])^2} + \frac{(y-\boldsymbol{\mathrm{y0}}[i])^2}{(\boldsymbol{\mathrm{sigma\_y}}[i])^2}\right)\right) + \boldsymbol{\mathrm{B}}[i].
	\]
	...
	\matlabs{opts.INITIAL\_CONDITION.sigms\_y = [1e-1, 1e-1];}
	...
	This parameter is a struct with fields: REACTIONS, SPECIES\_COEFF, BC, and \\ UPDATE\_FREQUENCY.

	\paragraph{REACTIONS} is a cell array of strings. It is used to indicate which chemical reactions (contained in the kinetic scheme specified with \seclink{chemicalmodel}{CHEMICAL\_MODEL}) contribute to the ionization rate of neutral molecules by electron impact. The total ionization rate, $S_i$, will be computed as the sum of the reaction rates of the reactions specified. For example, you could set
	
	\matlabs{opts.PHOTOIONIZATION.REACTIONS = \{}
	
	\hspace{40pt} \matlabs{'e + N2 -> 2e + N2+';} 
	
	\hspace{40pt} \matlabs{"e + O2 -> 2e + O2+"\};}
	
	
	\paragraph{SPECIES\_COEFF} is used to distribute the total photoionization rate, $S_{ph}$, among different species.
	SPECIES\_COEFF is a cell array with two columns. The first column contains the names of the positive ions produced (as strings) and the second column contains the weight associated to each positive ion.
	
	Considering the previous example of PHOTOIONIZATION.REACTIONS, you may want to split $S_{ph}$ between \ce{N2+} and \ce{O2+} with different weights.
	You could set
	
	\matlabs{opts.PHOTOIONIZATION.SPECIES\_COEFF = \{}
	
	\hspace{40pt} \matlabs{"N2+",0.8;}
	
	\hspace{40pt} \matlabs{"O2+",0.2\};}
	
	\noindent to add $0.8 \, S_{ph}$ to the production rate of \ce{N2+} and to add $0.2 \, S_{ph}$ to the production rate of \ce{O2+}. $S_{ph}$ will be automatically added to the production rate of electrons.
	
	\paragraph{BC} is used to set the boundary conditions for the Helmholtz equation
	\begin{equation*}
		\nabla^2 S_{ph}^{(j)} - (\lambda_j p_{\ce{O2}})^2 S_{ph}^{(j)} = -A_j p_{\ce{O2}}^2 I .
	\end{equation*}
	
	BC must be a vector containing as many elements as there are BoundaryIDs (see Section \ref{sec:NemericalIdentifiers}). Each element of the vector can be set to 0, corresponding to a \textbf{zero} Dirichlet boundary condition at the associated external boundary, or 1, corresponding to a \textbf{zero} Neumann condition. 
	For example, you could set
	
	\matlabs{opts.PHOTOIONIZATION.BC = [1, 1, 0, 1];}
	
	
	\paragraph{UPDATE\_FREQUENCY} is an integer used to specify the frequency at which the photoionization rate $S_{ph}$ is updated. 

	\noindent Setting 
	
	\matlabs{opts.PHOTOIONIZATION.UPDATE\_FREQUENCY = 1;} 
	
	\noindent means that the rate $S_{ph}$ will be recomputed at each time instant contained inside \\ \seclink{timeinst}{TIME\_INSTANTS}.
	
	\noindent Setting 
	
	\matlabs{opts.PHOTOIONIZATION.UPDATE\_FREQUENCY = 5;} 
	
	\noindent means that the rate $S_{ph}$ will be recomputed once every 5 time instants.
	...
	\subsection{CONST\_SPECIES}
	\label{sec:constspecies}
	...
	The first column should contain the name of the species (as strings) to treat as constant. All the species should appear inside the kinetic scheme specified with the \seclink{chemicalmodel}{CHEMICAL\_MODEL} parameter.
	
	The second column contains the constant value of the associated species, that can be an absolute value or a relative value with respect to the gas density, depending on the content of the third column.
	If the third column contains the string \matlabs{"abs"}, the number in the corresponding second column will be interpreted as a constant number density (in \unit{\per\cubic\meter}). If the third column contains the string \matlabs{"rel"}, the number in the corresponding second column will be interpreted as a fraction of the gas density \matlabs{Ngas}.
	
	For example, you could impose a constant number density for the species \matlabs{"A"} equal to 80\% of the total gas density, and a constant number density for the species \matlabs{"B"} equal to \SI{1e15}{\per\cubic\meter} by setting 
	
	\matlabs{opts.CONST\_SPECIES = \{}
			
	\hspace{40pt} \matlabs{"A", 0.8, "rel";}
		
	\hspace{40pt} \matlabs{"B", 1e15, "abs"\};}
	
	\noindent Note that all the species defined in \seclink{constspecies}{CONST\_SPECIES} must be removed from the input parameters \seclink{bcflag}{BC\_FLAG}, \seclink{bcval}{BC\_VAL}, \seclink{initial}{INITIAL\_CONDITION}, \seclink{mu}{MU}, \seclink{d}{D}, \seclink{vthcoeff}{V\_TH\_COEFF}, and \seclink{constomega}{CONST\_OMEGA}, since they will not be part of the degrees of freedom of the simulation.
	
	\vspace{10pt}
	
	\noindent If no species need to be considered as constant, you can avoid to define \seclink{constspecies}{CONST\_SPECIES}.
	
	
	\subsection{LOKI\_INPUT}
	\label{sec:lokiinput}
	This parameter is a string used to specify the \textbf{.in} input file for LoKI-B.
	To run a new LoKI-B input file, you must first of all create a valid one (we refer the reader to \href{https://github.com/LoKI-Suite/LoKI-B/tree/master/Documentation}{LoKI-B documentation}), and place it inside the \textbf{Input/} folder of the LoKI-B directory (note that it is not possible to put the file inside a sub-folder). You can then set \seclink{lokiinput}{LOKI\_INPUT} to the name of the input file, removing from the name the \textbf{.in} extension. 
	
	For example, if you created a LoKI-B input file named \textbf{my\_loki\_input.in} and you put it inside the \textbf{Input/} directory (already present in LoKI-B distribution), you should set 
	
	\matlabs{opts.LOKI\_INPUT = "my\_loki\_input";}
	
	\vspace{10pt}
	
	\noindent To avoid running again the same file in the future, and load directly the results, it is suggested to save the results directly from the LoKI-B input file, by putting these settings at the end:
	
	\lstinputlisting{save_settings.txt}
	
	\noindent Note that the text \matlabs{my_loki_input} after the \matlabs{folder} field should be replaced with the actual name of the \textbf{.in} file (\matlabs{my_loki_input} was used here to be consistent with the previous example).  
	If the shown settings are added to the \textbf{.in} file, the next time a simulation is run with the option
	
	\matlabs{opts.LOKI\_INPUT = "my\_loki\_input";}
	
	\noindent the results will be loaded directly without the need to run again LoKI-B.
	
	The input file for LoKI-B, \textbf{Air.in}, for a mixture of \ce{N2}(79 \%) - \ce{O2}(21 \%), is already provided with Merlino2D.
