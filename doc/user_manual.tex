...
\begin{document}
	...
	If you wish to use the functionalities of the LoKI-B Boltzmann solver, you should also have available on your computer \href{https://github.com/apopoli/Merlino}{Merlino0D}.
	...
	By running this script, you will be asked to select the \textbf{gmsh.exe} executable (that you should have previously installed), and the LoKI-B folder (that is contained inside Merlino0D main directory).
...
All the possible parameters have a default value that can be found inside \textbf{DefaultMerlino2Dinput.m}.
...
\item \matlabs{"flip\_y"}: if equal to 1, adds to the 2D plot the y-axis symmetric (very specific usage)
...
\matlabs{out\_pp = Load("my\_result.mat");}
	...
	\subsection{BC\_FLAG}
	\label{sec:bcflag}
	This parameter is a string matrix with \seclink{ns}{NS} rows and as many columns as there are BoundaryIDs (see Section \ref{sec:NemericalIdentifiers}). The $(i,j)$ element of this matrix contains a string specifying the boundary condition for the $i$ species at the $j$ boundary. The available conditions are:
	\begin{itemize}
	...
	\matlabs{opts.BC\_FLAG = ["GorinLike", "Flux", "Dirichlet";}
	
	\hspace{88pt} \matlabs{"GorinLike", "FreeDriftFlow", "Flux"];}

    \noindent Note that when using \matlabs{"GorinLike"} boundary conditions, the first species will automatically be interpreted as electrons and therefore secondary emission will be considered (with a coefficient \seclink{gammaII}{GAMMA\_II}).
	
	\subsection{BC\_VAL}
	\label{sec:bcval}
	This parameter is a function handle that returns a matrix with \seclink{ns}{NS} rows and as many columns as there are BoundaryIDs (see Section \ref{sec:NemericalIdentifiers}). It is used to specify in time the value for the \matlabs{"Dirichlet"} or \matlabs{"Flux"} boundary condition. Similarly to \seclink{bcflag}{BC\_FLAG}, the $(i,j)$ element of the \seclink{bcval}{BC\_VAL} matrix contains the desired value of number density (in \unit{\per\cubic\meter}) or flux density (in \unit{\per\square\meter\per\second}) for the $i$ species at the $j$ boundary, in the case of a \matlabs{"Dirichlet"} condition or a \matlabs{"Flux"} condition, respectively. 
	...
	\matlabs{opts.BC\_VAL = @(t)[NaN, 1e10*t, 1e15;}
	
	\hspace{105pt} \matlabs{NaN, NaN, t^2];}
	
	\noindent With this combination of parameters you are imposing a constant number density of \SI{1e5}{\per\cubic\meter} for the first species at the third boundary, and a time-dependent expression for computing the flux of the first species at the second boundary and of the second species at the third boundary. Notice that \matlabs{NaN} have been placed in \seclink{bcval}{BC\_VAL} in the positions corresponding to \matlabs{"GorinLike"} and \matlabs{"FreeDriftFlow"} flags. This was done to highlight that, for those species and boundaries where a \matlabs{"GorinLike"} or a \matlabs{"FreeDriftFlux"} condition has been used, the value inserted in the corresponding position of \seclink{bcval}{BC\_VAL} will be completely ignored.
	...
	If you wish to assign a uniform number density to each species across the entire domain, set \seclink{initial}{INITIAL\_CONDITION} to a vector with \seclink{ns}{NS} elements, where each element specifies the desired uniform number density for the corresponding species.
	
	\matlabs{opts.INITIAL\_CONDITION = [1e10, 1e12, 1e13, 1e25];}
	...
	If you wish to assign to each species a two-dimensional Gaussian distribution, set \seclink{initial}{INITIAL\_CONDITION} to a struct with fields
	\begin{itemize}
		\item A
		\item B
		\item x0
		...
		\item sigma\_y
	\end{itemize}
	each of them being a vector with \seclink{ns}{NS} elements. This will generate for the \textbf{ith} species a number density distribution
	\[
	...
	\]
	where the value of the parameters A, B, x0, y0, sigma\_x and sigma\_y is taken from the corresponding \textbf{i} element of the array. For example you could set
	...
	\subsection{S\_NAMES}
	\label{sec:snames}
	This parameter is a string array with \seclink{ns}{NS} elements, containing the names you wish to assign to each species.
	
	\matlabs{opts.S\_NAMES = ["e", "N2", "O2", "O4+"];}
	
	\noindent This information will mainly be used when plotting the results.
	...
	\noindent \seclink{ns}{NS} should be consistent with the dimension of \seclink{qs}{QS}, \seclink{mass}{MASS}, \seclink{mu}{MU}, \seclink{D}{D}, \seclink{vthcoeff}{V\_TH\_COEFF}, \seclink{constomega}{CONST\_OMEGA}, \seclink{snames}{S\_NAMES} and, only in the case of a Gaussian distribution, also \seclink{initial}{INITIAL\_CONDITION}. 
	
	\subsection{QS}
	\label{sec:qs}
	This parameter is a vector with \seclink{ns}{NS} elements, specifying the charge sign of of each species
	
	\matlabs{opts.QS = [-1,0,1,-1];}
	
	\noindent Be aware that the values of \seclink{qs}{QS} should be consistent with the species defined in the \seclink{chemicalmodel}{CHEMICAL\_MODEL}.
	
	\subsection{MASS}
	\label{sec:mass}
	This parameter is a vector with \seclink{ns}{NS} elements, specifying the mass of each species, in \unit{\kilo\gram}
	
	\matlabs{opts.MASS = [me, 0.032/Na, 0.028/Na, 3.75e-25];}
	
	\noindent The constants \matlabs{me} and \matlabs{Na}, representing the electron mass and the Avogadro number, respectively, can be useful for the initialization of this vector. Be aware that the values of \seclink{mass}{MASS} should be consistent with the species defined in the \seclink{chemicalmodel}{CHEMICAL\_MODEL}. This parameter is used for the computation of the thermal velocity.
	
	\subsection{MU}
	\label{sec:mu}
	This parameter is used to set the mobilities of the species, in \unit{\square\meter\per\volt\per\second}. \seclink{mu}{MU} must be a cell array with \seclink{ns}{NS} elements.
	Each element can be a number (if you wish to set a constant and uniform mobility) or a string. You should use a string to write an expression that will be used to compute the mobility.
	In the string, you can use any valid Matlab syntax to combine the available standard variables \textbf{E}, \textbf{Te}, \textbf{T} and \textbf{Ngas}, which correspond to the electric field in \unit{\Td}, the electron temperature in \unit{\electronvolt}, the gas temperature in K, and the gas density in \unit{\per\cubic\meter}, respectively. For example, you can set
	
	\matlabs{opts.MU = \{0.001, "exp(E/120)*T^(-0.5)", "Ngas*max(log(T),Te^(-0.2))"\};}
	
	\noindent to have a constant mobility for the first species and a mobility that is computed as a function of \textbf{E}, \textbf{Te}, \textbf{T}, and \textbf{Ngas} for the second and third species.
	...
	\matlabs{opts.MU = \{0.001, "exp(E/120)*T^(-0.5)", "my\_mob\_as\_func\_of\_Te(Te)"\};}
	...
	\matlabs{opts.MU = \{"Loki\_mu(E)/Ngas", 1e-4, "exp(E/120)*T^(-0.5)"\};}
	
	\subsection{D}
	\label{sec:d}
	This parameter is used to set the diffusion coefficient of the species, in \unit{\square\meter\per\second}. \seclink{D}{D} must be a cell array with \seclink{ns}{NS} elements.
	Each element can be a number (if you wish to set a constant and uniform value for the diffusion coefficient) or a string (if you wish to define an expression that will be used to compute the value of the diffusion coefficient). The exact same rules explained for the definition of a \seclink{mu}{MU} string expression (standard variables, csv variables and loki variables) also apply for \seclink{D}{D}. 
	
	The only difference is that, when using a string expression for \seclink{D}{D}, you can use a fifth standard variable, \textbf{mu}, in addition to the four previously described (\textbf{E}, \textbf{Te}, \textbf{T}, \textbf{Ngas}). With this variable, you can access the mobility of the \textbf{ith} species, and use it in the definition of the diffusion coefficient expression. Note that if you want to use the mobility of the species 7, 12, and 104, you should write in the string expression \matlabs{"mu7"}, \matlabs{"mu12"}, and \matlabs{"mu104"}, respectively. This standard variable is particularly useful when you want to use the Einstein relation to compute the diffusion coefficient. For example, you can set
	
	\matlabs{opts.D = \{"mu1*Te", "mu2*T/11600", "mu3*T/11600"\};}
	
	\noindent to use the Einstein relation for all three species (electrons being the first one). Alternatively, if a value was specified for \seclink{lokiinput}{LOKI\_INPUT}, you could set
	
	\matlabs{opts.D = \{"Loki\_D(E)/Ngas", "mu2*T/11600", "mu3*T/11600"\};}
	...
	This parameter should be a vector with as many elements as \seclink{ns}{NS}, each of them being a 0 (if the user wants to neglect the thermal velocity contribution) or a 1 (if the user wants to consider also the thermal velocity contribution).
	For example, in a simulation with \seclink{ns}{NS} = 6, setting
	
	\matlabs{opts.V\_TH\_COEFF = [0,0,0,1,1,1];}
	
	\noindent means that for the first 3 species the thermal velocity contribution to the boundary flux density will be neglected, while it will be considered for the latter 3 species.
	
	\subsection{CONST\_OMEGA}
	\label{sec:constomega}
	This parameter is used to set a constant source term for the species. \seclink{constomega}{CONST\_OMEGA} must be a \textbf{row} vector with \seclink{ns}{NS} elements. Each element will contain the constant and uniform value (in \unit{\per\cubic\meter\per\second}) of the source term to add to the $\partial n/\partial t$ of the corresponding species. Considering a case with electrons as the first species, and positive ions as second and third species, you could set
	
	\matlabs{opts.CONST\_OMEGA = [1e15, 5e14, 5e14, 0, 0, 0];}
	
	\noindent to mimic background ionization.
	...
	\noindent which means that no kinetic scheme will be used. This is equivalent to solving the drift-diffusion equations without considering the source term $\Omega$. If you want to use a kinetic scheme, you must first of all create a \textbf{.m} script inside the \textbf{kinetic/} folder. In this script, you should define the details of the kinetic scheme you wish to use. You can then assign to \seclink{chemicalmodel}{CHEMICAL\_MODEL} the name of the file you created. For example, if you created the script \textbf{my\_kinetic\_scheme.m}, you can set
	...
	A kinetic scheme script should contain 4 variables:
	\begin{itemize}
		\item \matlabs{species} must be a string array containing the names of the species. Note that these names will only be used to match the terms contained inside the reactions, while the names of the species used for plotting can be assigned using \seclink{snames}{S\_NAMES}. It is important that the order of the species matches between the kinetic scheme script and the parameters \seclink{qs}{QS}, \seclink{mass}{MASS}, \seclink{mu}{MU}, \seclink{D}{D}, \seclink{vthcoeff}{V\_TH\_COEFF}, \seclink{constomega}{CONST\_OMEGA} and \seclink{snames}{S\_NAMES}. \textbf{The first species must be electrons}. 
		
		\item \matlabs{reactions} must be a cell array, with two columns and as many rows as the number of chemical reactions included in the kinetic scheme. The first column should contain the definition of the reactions, the second should contain the definition of the corresponding rate coefficients (in \unit{\cubic\meter\per\second} or \unit{\metre\tothe{6}\per\second}). 
		...
		Note that every chemical species appearing in the reactions should be present in \matlabs{species}. For instance, referring to the previous example, \matlabs{species} should contain \matlabs{"A"}, \matlabs{"B"}, \matlabs{"C"}, \matlabs{"D"}, \matlabs{"E"}, and \matlabs{"C+"}. 
		
		Each element of the second column can be a number (if you wish to set a constant and uniform value for the rate coefficient) or a string (if you wish to define an expression that will be used to compute the rate coefficient). 
		...
		To achieve this, you need to set the corresponding element of the second column of \matlabs{reactions} to a specific string, that will allow Merlino0D to select the right rate computed by LoKI-B.
		...
		\item \matlabs{const\_species} must be a vector containing the indices of the species defined in \matlabs{species} that you wish to keep uniform and constant during the simulation. If you do not want to consider any species constant, you can set \matlabs{const\_species} to \matlabs{[]}.
		Note that if you defined 10 species, but then you set 3 of them as constant, you have to set \seclink{ns}{NS} = 10 - 3 = 7. This should be consistent also with \seclink{qs}{QS}, \seclink{mass}{MASS}, \seclink{mu}{MU}, \seclink{D}{D}, \seclink{vthcoeff}{V\_TH\_COEFF}, \seclink{constomega}{CONST\_OMEGA} and \seclink{snames}{S\_NAMES}.
		
		\item \matlabs{const\_vals} must be a vector containing the constant values you want to assign to the species you set as constant. \matlabs{const\_vals} should have the same number of elements as \matlabs{const\_species}. If no species have been set as constants, set \matlabs{const\_vals} to \matlabs{[]}. Note that you have available the variable \matlabs{Ngas} when creating \matlabs{const\_vals}. This means that you can do something like
		
		\matlabs{const\_vals = [0.8*Ngas, 0.2*Ngas];}
		
	\end{itemize}
	In Merlino2D some kinetic schemes for air are already provided inside the folder \textbf{kinetic/} and can be used as reference for the creation of new scripts.
	
	\subsection{LOKI\_INPUT}
	\label{sec:lokiinput}
	This parameter is a string used to specify either the \textbf{.in} input file for LoKI-B, or the \textbf{.mat} file where LoKI results have been previously stored.
	To run a new LoKI-B input file, you must first of all create a valid one (we refer the reader to \href{https://github.com/LoKI-Suite/LoKI-B/tree/master/Documentation}{LoKI-B documentation}), and place it inside the \textbf{Input/} folder of the LoKI directory. You can then set \seclink{lokiinput}{LOKI\_INPUT} to the name of the input file. You must include in the name the \textbf{.in} extension. If the input file was placed in some sub-folder inside the \textbf{Input/} directory, you can add that to the name of the file. For example, if you created a LoKI input file named \textbf{my\_loki\_input.in} and you put it inside the sub-folder\textbf{ Merlino2D\_inputs/} inside the \textbf{Input/} directory (already present in LoKI-B distribution), you should set 
	
	\matlabs{opts.LOKI\_INPUT = "Merlino2D\_inputs/my\_loki\_input.in";}
	
	\noindent If you have already saved LoKI results in a previous simulation (using the \seclink{saveloki}{SAVE\_LOKI} parameter), \seclink{lokiinput}{LOKI\_INPUT} can be set to the name of the saving file. For example, if you previously saved LoKI results in the file \textbf{my\_save.mat}, you can set
	...
	The input file for LoKI-B, \textbf{Air.in}, for a mixture of \ce{N2}(79 \%) - \ce{O2}(21 \%), is already provided with Merlino2D. It should be moved inside the \textbf{Input/} sub-folder of the LoKI directory if you wish to use the Boltzmann solver functionalities.
